...
def get_data(code_currencies):
    ...
    url = f'https://cbr.ru/currency_base/dynamics/?UniDbQuery.Posted=True&UniDbQuery.so=1&UniDbQuery.mode=1&UniDbQuery.date_req1=&UniDbQuery.date_req2=&UniDbQuery.VAL_NM_RQ={code_currencies}&UniDbQuery.From=01.01.1992&UniDbQuery.To={today}'
    try:
        r = requests.get(url=url, headers=headers, timeout=5)
    except TooManyRedirects as e:
        print(f'{url} : {e}')
...
def create_table(name_table, columns):
    try:
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                f"CREATE TABLE IF NOT EXISTS {name_table}({columns[0]} DATE UNIQUE, {columns[1]} INTEGER, {columns[2]} REAL);")
...
def insert_data(name_table,columns, value):
    ...
    #insert data into a table
        insert = f"INSERT INTO {name_table} ({columns[0]}, {columns[1]}, {columns[2]}) VALUES ('{value[0]}',{int(value[1])}, {Decimal(value[2].replace(',', '.'))});"
        with connection.cursor() as cursor:
